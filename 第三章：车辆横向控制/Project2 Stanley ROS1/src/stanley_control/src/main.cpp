...
int main(int argc, char** argv) {
  ...
  while (getline(infile, s)) {
    ...
    word >> x;
    word >> y;
    double pt_x = std::atof(x.c_str()); //将字符串转换为浮点数
  ...
  //创建四个向量，分别用于存储头部角度、累积距离、曲率和曲率的变化率
  ...
  /*构建一个规划轨迹，将之前计算的参考线属性（X、Y坐标、头部角度、曲率等）填充到 TrajectoryPoint 结构中，
  并将这些点存储在 planning_published_trajectory 对象中的 trajectory_points 向量中*/
  ...
  //将规划轨迹中的每个点转换为ROS中的路径消息，并将这些消息存储在 reference_path 中，以便在ROS系统中进行可视化
  for (size_t i = 0; i < headings.size(); i++) {
  ...
  while (ros::ok()) {
    if(first_record_) { 
      ...
      control_cmd.reverse = false; //不进行倒车
