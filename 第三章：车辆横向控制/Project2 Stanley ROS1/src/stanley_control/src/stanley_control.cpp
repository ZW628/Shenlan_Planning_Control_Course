...
void StanleyController::LoadControlConf() {
  k_y_ = 0.5;
        ...
        void StanleyController::ComputeLateralErrors(const double x, const double y, const double theta, double &e_y, double &e_theta) 
        {
            ...
            auto trajectory_points = QueryNearestPointByPosition(x,y); //查询离车辆当前位置最近的轨迹点
            ...
            e_y = sin(delta)*PointDistanceSquare(trajectory_points, x, y); //横向误差
            cout << "横向误差："<< e_y <<endl;
            e_theta = theta-trajectory_points.heading; //航向误差
            if(e_theta > M_PI)
            {
