...
bool MPCController::ComputeControlCommand(
  ...
  MpcOsqp mpc_osqp(matrix_ad_, matrix_bd_, matrix_q_, matrix_r_, matrix_state_,
                   lower_bound, upper_bound, lower_state_bound,
...
void MPCController::UpdateMatrix(const VehicleState &vehicle_state) {
  ...
  v = std::max(vehicle_state.velocity,
               minimum_speed_protection_);  // 防止速度为0
  ...
  matrix_ad_ =
      (matrix_i - ts_ * 0.5 * matrix_a_).inverse() *  // 将状态矩阵A离散化
...
void MPCController::ComputeErrors(const double x, const double y,
  ...
  double heading_error = NormalizeAngle(
      theta -
      target_point
          .heading);  // 将参考轨迹点的朝向和当前车辆的朝向相减，再归化到[-M_PI,M_PI]之间
  ...
  station_error_ =
      -(dx * cos_target_heading + dy * sin_target_heading);  // 纵向误差计算

  speed_error_ = target_point.v -
                 linear_v * std::cos(abs(heading_error)) /
                     (1 - target_point.kappa * lateral_error);  // 速度误差计算
}
