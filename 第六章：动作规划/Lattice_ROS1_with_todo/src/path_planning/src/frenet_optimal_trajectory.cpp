    ...
    for (float di = 0; di <= 5; di += D_ROAD_W)
    {
        for (float ti = MINT; ti <= MAXT; ti += DT)
        {
            ...
            for (float vi = TARGET_SPEED - D_T_S * N_S_SAMPLE; vi < TARGET_SPEED + D_T_S * N_S_SAMPLE; vi += D_T_S)
            {
                ...
                fp_with_s.cd = KJ * Jp + KT * ti + KD * pow(fp_with_s.d.back(), 2); // 横向代价，横向加速度的平方和 Jp、路径规划时间 ti 以及横向偏差的平方
                // # square of diff from target speed
                float ds = pow(TARGET_SPEED - fp_with_s.s_d.back(), 2);
                fp_with_s.cv = KJ * Js + KT * ti + KD * ds;               // 纵向代价，纵向加速度的平方和 Js、路径规划时间 ti 以及速度偏差的平方
                fp_with_s.cf = KLAT * fp_with_s.cd + KLON * fp_with_s.cv; // 综合代价，是横向代价和纵向代价的线性组合
...
// 找到在给定条件下（车辆状态、起始位置、障碍物等）成本最小的Frenet路径，并将其转换为全局路径返回
