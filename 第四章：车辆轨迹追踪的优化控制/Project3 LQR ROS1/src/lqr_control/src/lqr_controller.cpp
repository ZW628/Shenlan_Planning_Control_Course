  ...
  namespace control
  {
    ...
    bool LqrController::ComputeControlCommand(const VehicleState &localization, const TrajectoryData &planning_published_trajectory, ControlCmd &cmd)
    {
      ...
      */
      ...
      double steer_angle_feedback = -(matrix_k_ * matrix_state_)(0, 0); //(0, 0) 选择结果矩阵的第一个元素
    ...
    {
      // 创建一个大小与 matrix_a_ 相同的单位矩阵，用于后续离散化操作
    ...
    void LqrController::SolveLQRProblem(const Matrix &A, const Matrix &B, const Matrix &Q, const Matrix &R, const double tolerance, const uint max_num_iteration, Matrix *ptr_K)
    {
      ...
      Eigen::MatrixXd AT = A.transpose(); // 该函数返回矩阵 A 的转置矩阵
      Eigen::MatrixXd BT = B.transpose();
      double error = std::numeric_limits<double>::max(); // 将 error 初始化为 double 类型的最大值
      Eigen::MatrixXd P = Q;                             // 初始化 P 矩阵为 Q
      for (uint i = 0; i <= max_num_iteration; i++)
      {

        Eigen::MatrixXd P_Next = AT * P * A - AT * P * B * (R + BT * P * B).inverse() * BT * P * A + Q;
        // 计算两个矩阵 P_Next 和 P 之间差值的绝对值的最大值
        // 通过监控误差的变化，可以判断算法是否收敛，并在达到期望的精度时提前退出
        error = fabs((P_Next - P).maxCoeff());
        P = P_Next;            // 更新 P 矩阵
        if (error < tolerance) // 如果误差小于容忍度，退出迭代
        {
          break;
        }
      }
